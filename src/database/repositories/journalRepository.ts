...
export const journalRepository = {
  async getAll(): Promise<JournalEntry[]> {
    ...
    const rows = await db.getAllAsync<any>('SELECT * FROM journal_entries ORDER BY date DESC, created_at DESC');
  ...
  async update(id: string, updates: Partial<Omit<JournalEntry, 'id' | 'createdAt' | 'updatedAt'>>): Promise<void> {
    const db = await getDatabase();
    const now = new Date().toISOString();
    ...
    if (updates.isScanned !== undefined) { fields.push('is_scanned = ?'); values.push(updates.isScanned ? 1 : 0); }
    if (updates.originalImageUri !== undefined) { fields.push('original_image_uri = ?'); values.push(updates.originalImageUri); }
    if (updates.ocrConfidence !== undefined) { fields.push('ocr_confidence = ?'); values.push(updates.ocrConfidence); }
  ...
  async getAllTags(): Promise<string[]> {
    const db = await getDatabase();
    const rows = await db.getAllAsync<{ tags: string | null }>('SELECT tags FROM journal_entries WHERE tags IS NOT NULL');
